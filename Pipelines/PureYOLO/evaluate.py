...
from model import DEFAULT_YOLO_PATH, TASK_NAMES, OpenVocabDetectionFeatureExtractor, TaskAwareRelevanceHead, gather_topk_like_model
...
DEFAULTS = dict(
    ...
    weights_path=os.path.join(CURRENT_DIR, "weights", "v1c_relevance_best.pt"),
...
def evaluate(cfg):
    ...
    if os.path.exists(cfg["weights_path"]):
        ...
        raise FileNotFoundError(f"V1C weights not found: {cfg['weights_path']}")
    model.eval()
    ...
    records = []
    per_task_v1c = {t: {"total": 0, "correct": {iou: 0 for iou in iou_thresholds}} for t in range(1, 15)}
    per_task_yolo = {t: {"total": 0, "correct": {iou: 0 for iou in iou_thresholds}} for t in range(1, 15)}

    with torch.no_grad():
        for batch in tqdm(loader, desc="Evaluating V1C"):
            ...
            for b in range(topk_boxes.size(0)):
                ...
                per_task_v1c[task_id]["total"] += 1
                per_task_yolo[task_id]["total"] += 1
                preferred_xyxy = xywh_to_xyxy(preferred)

                if cfg["score_mode"] == "fused":
                    rank_scores = probs[b] * topk_scores[b]
                else:
                    rank_scores = probs[b]
                rank_scores = rank_scores.masked_fill(~valid, -1.0)

                best_idx = torch.argmax(rank_scores).item()
                v1c_box = topk_boxes[b, best_idx].unsqueeze(0)
                v1c_max_iou = pairwise_iou_xyxy(v1c_box, preferred_xyxy).max().item()
                hit_dict = {iou: v1c_max_iou >= iou for iou in iou_thresholds}
                ...
                for iou in iou_thresholds:
                    if hit_dict[iou]:
                        per_task_v1c[task_id]["correct"][iou] += 1
    ...
    for iou in iou_thresholds:
        tp_v1c = sum(r[1][iou] for r in records)
        tp_yolo = sum(r[2][iou] for r in records)
        top1_v1c[iou] = tp_v1c / total if total else 0.0
    ...
    width = 0.38
    v1c_acc = [per_task_results[t]["v1c_acc"][primary_iou] * 100 for t in range(1, 15)]
    yolo_acc = [per_task_results[t]["yolo_acc"][primary_iou] * 100 for t in range(1, 15)]
    ax.bar(x - width / 2, v1c_acc, width, label="V1C Open-Vocab Relevance", color="#4C9BE8", zorder=3)
    ax.bar(x + width / 2, yolo_acc, width, label="YOLO-only", color="#E8784C", zorder=3)
    ax.set_xlabel("Task")
    ax.set_ylabel("Top-1 Accuracy (%)")
    ax.set_title("SATAY-ViT V1C vs YOLO-only - Per-Task Accuracy")
    ax.set_xticks(x)
    ...
    fig.text(
        0.5, 0.97,
        f"Overall: V1C {top1_v1c[primary_iou] * 100:.1f}% | YOLO {top1_yolo[primary_iou] * 100:.1f}% | mAP {maps[primary_iou] * 100:.1f}%",
