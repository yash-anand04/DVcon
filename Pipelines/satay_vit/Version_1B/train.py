...
def train(cfg):
    ...
    # NOTE: Do NOT call extractor.eval() — ultralytics YOLO overrides .train() to launch
    # a full training run, so PyTorch's module.train(False) chain triggers it accidentally.
    # We only need ResNet in eval mode; YOLO is already frozen via @torch.no_grad() in forward.
    ...
    if os.path.exists(latest_path):
        ...
        history_path = os.path.join(cfg["weights_dir"], "training_history.json")
        if os.path.exists(history_path):
            with open(history_path, "r") as f:
                history = json.load(f)
                train_history = history.get("train", [])
                val_history = history.get("val", [])
                best_val = history.get("best_val_loss", best_val)
