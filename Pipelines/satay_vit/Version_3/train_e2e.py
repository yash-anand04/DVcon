"""
train_e2e.py  –  End-to-end SATAY-ViT Training Script
======================================================
Trains the full SATAYViT_E2E model (YOLO backbone + FPN + ME-ViT head)
jointly on the COCO-Tasks dataset using BCE heatmap loss.
...
DEFAULTS = dict(
    ...
    weights_dir   = "e:/DVcon/DVcon/Pipelines/satay_vit/Version_3/weights_e2e",
    epochs        = 15,
    batch         = 16,
    lr            = 5e-5,           # conservative: we're fine-tuning a pretrained backbone
...
def train(cfg: dict):
    ...
    train_ds = COCOTasksDataset(cfg["data_root"], split="train", grid_size=32)
    val_ds   = COCOTasksDataset(cfg["data_root"], split="test", grid_size=32)
    ...
    # ── model ────────────────────────────────────────────────────
    model = SATAYViT_E2E(embed_dim=cfg["embed_dim"], top_k=cfg["top_k"]).to(device)
    ...
    for epoch in range(cfg["epochs"]):
        ...
        pbar = tqdm(train_loader,
                    desc=f"Epoch {epoch+1:>2}/{cfg['epochs']} [Train]")
        for batch in pbar:
        ...
        with torch.no_grad():
            for batch in tqdm(val_loader,
                              desc=f"Epoch {epoch+1:>2}/{cfg['epochs']} [Valid]"):
                imgs     = batch["image"].to(device)
                tasks    = batch["task_id"].to(device)
                gt_hmaps = batch["heatmap"].to(device)
