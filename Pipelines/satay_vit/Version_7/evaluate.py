"""
evaluate.py  –  SATAY-ViT V7 Evaluation
========================================
...
DEFAULTS = dict(
    ...
    weights        = os.path.join(CURRENT_DIR, "weights", "v7_best.pt"),
...
def evaluate(cfg):
    ...
    model    = SATAYViT_V7(checkpoint=cfg["yolo_model"]).to(device)
    ...
    if os.path.exists(cfg["weights"]):
        ...
        raise FileNotFoundError(f"V7 weights not found: {cfg['weights']}")
    model.eval()
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc="Evaluating V7"):
            ...
            for b in range(img_tensors.shape[0]):
                ...
                v7_box   = boxes_b[best_v7].unsqueeze(0)
                yolo_box = boxes_b[best_yolo].unsqueeze(0)

                v7_iou   = pairwise_iou_xyxy(v7_box,   preferred_xyxy).max().item()
                yolo_iou = pairwise_iou_xyxy(yolo_box, preferred_xyxy).max().item()

                hit_dict      = {iou: v7_iou   >= iou for iou in iou_thresholds}
    ...
    for t in range(1, 15):
        ...
        per_task_results[t] = {
            ...
            "total":     n,
            "v7_acc":    {iou: per_task_v7[t]["correct"][iou]   / n if n else 0.0 for iou in iou_thresholds},
    ...
    print("\n" + "=" * 70)
    print("  SATAY-ViT V7 EVALUATION RESULTS")
    print("=" * 70)
    print(f"  Total samples evaluated : {total:,}")
    for iou in iou_thresholds:
        print(f"  Top-1 V7   @ IoU {iou:.2f}  : {top1_v7[iou]*100:.2f}%")
    print(f"  Top-1 YOLO @ IoU {primary:.2f}  : {top1_yolo[primary]*100:.2f}%")
    for iou in iou_thresholds:
        print(f"  mAP @ IoU  {iou:.2f}        : {maps[iou]*100:.2f}%")
    print("-" * 70)
    print(f"  {'Task':<30} {'V7':>8}   {'YOLO':>8}")
    print("-" * 70)
    for t in range(1, 15):
        n  = per_task_results[t]["total"]
        v6 = per_task_results[t]["v7_acc"][primary] * 100
    ...
    width    = 0.38
    v7_accs  = [per_task_results[t]["v7_acc"][primary]   * 100 for t in range(1, 15)]
    yo_accs  = [per_task_results[t]["yolo_acc"][primary] * 100 for t in range(1, 15)]
    ax.bar(x - width / 2, v7_accs, width, label="V7 RoI-Align", color="#C84B0E", zorder=3)
    ax.bar(x + width / 2, yo_accs, width, label="YOLO-only",     color="#88BBE8", zorder=3)
    ax.set_xlabel("Task")
    ax.set_ylabel("Top-1 Accuracy (%)")
    ax.set_title("SATAY-ViT V7 vs YOLO-only — Per-Task Accuracy")
    ax.set_xticks(x)
    ...
    fig.text(
        0.5, 0.97,
        f"Overall: V7 {top1_v7[primary]*100:.1f}%  |  YOLO {top1_yolo[primary]*100:.1f}%  |  mAP@{primary} {maps[primary]*100:.1f}%",
    ...
    if os.path.exists(hist_path):
        ...
        if train_l:
            ...
            fig, ax = plt.subplots(figsize=(max(8, len(train_l) * 0.6), 5))
            ax.plot(steps, train_l, "o-", color="#C84B0E", label="Train loss")
            ax.plot(steps, val_l,   "s--", color="#4C9BE8", label="Val loss")
            if labels:
            ...
            ax.set_title("V7 Training / Validation Loss")
            ax.legend()
