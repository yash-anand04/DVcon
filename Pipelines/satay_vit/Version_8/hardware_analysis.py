"""
hardware_analysis.py  –  V8 FPGA Resource Estimator
=====================================================
Profiles SATAYViT_V8 for:
...
from model import SATAYViT_V8, DEFAULT_YOLO_PATH
...
def analyse(n_detections=10):
    ...
    """
    model = SATAYViT_V8(checkpoint=DEFAULT_YOLO_PATH)
    model.eval()
    ...
    # Biggest layer in V8 backbone: C3k2 at layer 8, ~256 ch
    ...
    div = "─" * 68
    print(f"\n{'='*68}")
    print("  SATAY-ViT V8  —  FPGA Hardware Resource Analysis")
    ...
    rows = [
        ...
        ("V8 (ours)","2.56 M",     f"{fmt(w_int8)}", f"{fmt(total_bram_needed)}", "Yes ✓", "60.1%"),
